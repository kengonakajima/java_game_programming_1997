/*
   ...
   対応OSは、Windows 95,各種UNIXです。
   JDK1.1用のプログラムですが、JDK1.1と記載してある行をコメントアウトし、
   その近くのJDK1.0.2と書いてある行と入れかえると、JDK1.0.2でもコンパ
   イルすることができます。
   ...
   を指定します。
   portには、使用するポートを指定します。省略すると80番になります。
...
class httpd_child implements Runnable
{
	...
	public void run()
	{
		try{
			...
			String ftmp = new String( b , 0 , first_space + 1 ,
									 second_space - first_space -1); //JDK1.0.2
	...
	void writeMsg( String s ) throws IOException
	{
		...
		//b = s.getBytes();  // JDK1.1
		s.getBytes( 0 , s.length() , b , 0 ); // JDK1.0.2
